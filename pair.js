...
module.exports = {
  ...
  onStart: async function ({ api, event, usersData }) {
    ...
    try {
      ...
      const threadData = await api.getThreadInfo(
        event.threadID
      );
      ...
      function normalizeGender(gender) {
        if (typeof gender === "number") {
          if ([1, 3, 8].includes(gender)) return "FEMALE";
          if ([2, 4, 9].includes(gender)) return "MALE";
          return null;
        }
        ...
        if ([1, 3, 8].includes(number)) return "FEMALE";
        if ([2, 4, 9].includes(number)) return "MALE";
      ...
      async function getFreshGender(
        ...
        try {
          ...
          console.log(
            `[PAIR] Failed to fetch gender for ${userID}:`,
            error.message
          );
      ...
      if (!myData) {
        return api.sendMessage(
          "❌ Your user information was not found in this group.",
          event.threadID
        );
      }
      ...
      if (!myGender) {
        return api.sendMessage(
          "⚠️ Gender data not available for matching.",
          event.threadID
        );
      }
      ...
      const selectedMatchID =
        getUserID(selectedMatch);

      if (!selectedMatchID) {
        return api.sendMessage(
          "❌ Failed to get matched user's ID.",
          event.threadID
        );
      }
      ...
      if (!matchName) {
        try {
          ...
          console.log(
            `[PAIR] Failed to fetch matched user name ${selectedMatchID}:`,
            error.message
          );
        }
      }
      ...
      if (!matchName && usersData) {
        try {
          ...
          if (matchUserData) {
            matchName =
              matchUserData.name ||
              matchUserData.fullName ||
              matchUserData.displayName ||
              "";
          }
        } catch (error) {
          console.log(
            `[PAIR] usersData match error ${selectedMatchID}:`,
            error.message
          );
        }
      }
      ...
      console.log(
        `[PAIR] Match UID: ${selectedMatchID} | Name: ${matchName}`
      );
      ...
      const backgrounds = [
        ...
        },
        {
          ...
          type: "normal",
          pos: [
            { x: 111, y: 175, w: 330, h: 330 },
            { x: 1018, y: 173, w: 330, h: 330 }
          ]
        },
        {
          id: 6,
        ...
        },
        {
          ...
          type: "normal",
          pos: [
            { x: 120, y: 170, w: 300, h: 300 },
            { x: null, y: 170, w: 300, h: 300 }
          ]
        },
        {
          id: 8,
      ...
      if (
        ...
        if (
          ...
          drawCircle(
            ...
            selectedBg.pos[0].x,
            selectedBg.pos[0].y,
            selectedBg.pos[0].size
          );

          const x2 =
            selectedBg.pos[1].x !== null
              ? selectedBg.pos[1].x
              : canvas.width -
